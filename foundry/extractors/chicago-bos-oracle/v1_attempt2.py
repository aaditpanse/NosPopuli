...
def _clean_title(t):
    t = _clean(t)
    # drop a trailing legislative file number in parentheses to aid joining
...
def _parse_tally(block):
    """Return (counts, positions, anchor) or (None, {}, None)."""
    ...
    if m:
        ...
        for n in nn:
            pos[n] = "no"
        return {"aye": yc, "no": nc}, pos, m.start()
    m = VOTE_NUM_RE.search(block)
    if m:
        return {"aye": int(m.group("a")), "no": int(m.group("b"))}, {}, m.start()
...
def _parse_result(block):
    ...
    if pm and fm:
        # final outcome wins (latest in text)
...
def _outcome_for_block(block, doc_url, item_key, title):
    counts, positions, tally_anchor = _parse_tally(block)
    ...
    anchor = tally_anchor if tally_anchor is not None else kw_anchor
    if anchor is None:
        anchor = 0
...
def _parse_document(text, doc_url):
    ...
    for m in TALLY_RE.finditer(text):
        ...
        key = fnos[-1].lower() if fnos else None
        if key is None:
            continue
        block = text[max(0, m.start() - 400):min(len(text), m.end() + 100)]
        title = _clean_title(_clean(pre[-200:]))
        entry = _outcome_for_block(block, doc_url, key, title or key)
        if entry is not None:
            items.setdefault(key, []).append(entry)
    return items
...
def extract(rt, args):
    ...
    for iso in dates:
        ...
        if covered >= max_meetings:
            # has a document but beyond coverage window; not "missing".
            continue
