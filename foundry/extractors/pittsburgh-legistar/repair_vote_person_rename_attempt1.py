...
Repair notes (v3):
  * The Legistar `/votes` payload renamed its person field: the per-member
    voter name now arrives as `VotePersonFullName` (the old `VotePersonName`
    key is gone), which silently emptied every vote_event's `positions`/
    `counts`.  We now read `VotePersonFullName` (with a fallback to the
    legacy key for safety).
  * `positions` are emitted as `[member, position]` pairs, sorted by member
    name, to match the golden set; `counts` are derived from those pairs.
"""
...
def _vote_person_name(v):
    """Person name from a vote row, tolerating the renamed source field."""
    return v.get("VotePersonFullName") or v.get("VotePersonName")
...
def extract(fetch_json, event_ids):
    ...
    for eid in event_ids:
        ...
        for it in items:
            ...
            pairs = []
            for v in votes:
                name = _vote_person_name(v)
                pid = v.get("VotePersonId")
                record_person(pid, name)
                pos = _norm_position(v.get("VoteValueName"))
                if pos and name:
                    pairs.append((name, pos))

            # deterministic ordering by member name
            pairs.sort(key=lambda p: p[0])

            positions = [[name, pos] for name, pos in pairs]
            counts = {}
            for _name, pos in pairs:
