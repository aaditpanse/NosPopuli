...
Repair notes (v3):
  ...
  * `positions` remain schema v1 `{"member", "position"}` objects, sorted by
    member name; `counts` are derived from those objects.
"""
...
def extract(fetch_json, event_ids):
    ...
    for eid in event_ids:
        ...
        for it in items:
            ...
            positions = [{"member": name, "position": pos} for name, pos in pairs]
            counts = {}
