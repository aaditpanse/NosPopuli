...
Repair note (v3): the upstream source changed its date serialization from
ISO-8601 ("2026-06-30T00:00:00") to US month/day/year with a clock
component ("06/30/2026 12:00:00 AM"). `_date` now normalizes both forms to
YYYY-MM-DD so meetings match the golden set again.
"""
...
# ISO-8601 leading date: 2026-06-30T00:00:00 / 2026-06-30 ...
_ISO_DATE_RE = re.compile(r"^(\d{4})-(\d{2})-(\d{2})")
# US date: 06/30/2026 [12:00:00 AM]
...
def _date(value):
    """Normalize a Legistar date to 'YYYY-MM-DD'; None-safe.

    Handles both the historical ISO-8601 form ('2026-06-30T00:00:00') and
    the current US form with a clock component ('06/30/2026 12:00:00 AM').
    """
    if not value:
        return None
    s = str(value).strip()
    ...
    # Unrecognized shape: fall back to the first 10 chars (previous behavior).
