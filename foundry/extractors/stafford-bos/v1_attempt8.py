...
lists meetings; minutes PDFs are streamed as text via
`/v1/Meetings/GetMeetingFileStream(...)`. Votes are recorded in the minutes
as explicit tally blocks, e.g.:
...
Enumeration realities (confirmed from the fetch trace):
  * `/v1/Events` PAGES its results and the server caps page size at ~15 rows
    regardless of `$top`. We therefore FOLLOW `@odata.nextLink` until enough
    past Board-of-Supervisors meetings with minutes are reachable. The
    calendar is shared by many bodies and published a year+ ahead, so we also
    server-side `$filter=startDateTime lt <today>` and order newest-first.
  * CivicClerk can reuse one minutes file across events, so the meeting date
    is verified against the date printed INSIDE the document; mismatches are
    dropped and duplicate dates are collapsed.
"""
...
MAX_LIST_PAGES = 60   # listing fetches are cheap JSON; bounds runtime
...
def _doc_date(text):
    """Return YYYY-MM-DD printed near the head of the minutes, or None."""
    head = text[:800]
    m = DOC_DATE_RE.search(head)
    if not m:
        m = DOC_DATE_RE.search(text)
    if not m:
        return None
...
def _next_link(data):
    if isinstance(data, dict):
        for k in ("@odata.nextLink", "@odata.nextlink", "nextLink"):
            v = data.get(k)
...
def _is_bos(ev):
    return BODY_MATCH in _event_body(ev).lower()


def _get_events(rt, need):
    """Page through /v1/Events (following @odata.nextLink) collecting events,
    stopping once `need` past Board-of-Supervisors events are gathered."""
    today = date.today().isoformat()
    filt = f"startDateTime lt {today}T00:00:00Z"

    # ops patch 2026-07-12: this OData server treats $top as a TOTAL cap
    # across the nextLink chain (pages stay 15 rows); 50 reached only ~3
    # months back. Raised to 600 so deep backfills can enumerate.
    collected = []
    url = EVENTS_URL
    params = {"$filter": filt, "$orderby": "startDateTime desc", "$top": "600"}
    seen_links = set()

    for _ in range(MAX_LIST_PAGES):
        try:
            data = rt.fetch_json(url, params=params)
        except Exception:
            break
        ...
        if sum(1 for e in collected if _is_bos(e)) >= need:
            break
        nxt = _next_link(data)
        if not nxt or nxt in seen_links:
            break
        seen_links.add(nxt)
        url = nxt

    # First page may have used server-side filtering; if we somehow got
    # nothing, retry once without the filter.
    if not collected:
        try:
            data = rt.fetch_json(
                EVENTS_URL,
                params={"$orderby": "startDateTime desc", "$top": "600"},
            )
            collected = _events_from(data)
        except Exception:
            collected = []
    return collected
...
def extract(rt, args):
    ...
    # Collect a generous buffer of BOS events (some past events won't yet
    # have minutes posted / parseable votes).
    need = max(max_meetings * 4, 8)
    events = _get_events(rt, need)
    if not events:
        run_meta["row_counts"] = {k: 0 for k in records}
        return records, run_meta

    bos = [e for e in events if _is_bos(e)]
    if bos:
        events = bos

    # Newest first (server already orders, but be defensive).
    events.sort(key=lambda e: (_event_date(e) or ""), reverse=True)
    ...
    seen_dates = set()
    seen_fids = set()
    fetch_budget = max(max_meetings * 8, 40)
    fetched = 0
    kept = 0

    for ev in events:
        if kept >= max_meetings or fetched >= fetch_budget:
            break
        try:
            d = _event_date(ev)
            eid = _event_id(ev)
            ...
            # Verify the document's printed date matches this event; drop
            # reused/mismatched minutes files.
            inner = _doc_date(text)
