...
CivicClerk reuses the same minutes file across several events, so the meeting
date is verified against the date printed INSIDE the document; mismatches are
dropped (fixes gate/duplicate_document).
"""
...
def _get_events(rt, top):
    ...
    # NOTE: $expand=publishedFiles returns an empty body on this tenant;
    # fetch files separately per event instead.
    param_sets = [
...
def extract(rt, args):
    ...
    for ev in events:
        ...
        try:
            ...
            # Verify the date printed inside the minutes matches the event
            # date. CivicClerk reuses one minutes file across events; drop
            # the copies whose internal date does not match (gate fix).
