...
lists meetings (files inline as `publishedFiles`); minutes PDFs are streamed
as text via `/v1/Meetings/GetMeetingFileStream(...)`. Votes are recorded in
the minutes as explicit tally blocks, e.g.:
...
Two enumeration realities drive the design:
  * The event calendar is published a year+ ahead across many bodies, and it
    is sorted newest-first — so a small $top window is dominated by future
    non-BOS events. We server-side filter to past events and request a wide
    window so enough past BOS meetings-with-minutes are reachable.
  * CivicClerk reuses one minutes file across several events, so the meeting
    date is verified against the date printed INSIDE the document; mismatches
    are dropped (fixes gate/duplicate_document).
"""
...
def extract(rt, args):
    ...
    # The calendar (sorted newest-first) is dominated by future / non-BOS
    # events, so request a wide window to reach enough past BOS meetings.
    top = min(max(max_meetings * 30, 250), 500)
    ...
    seen_dates = set()
    seen_fids = set()
    # cap defensive fetch count so a pathological listing can't blow the
    # runtime budget, while still allowing plenty of headroom over the target
    ...
    for ev in events:
        ...
        try:
            ...
            # CivicClerk reuses one minutes file across events; keep only the
            # event whose date matches the date printed inside the document.
