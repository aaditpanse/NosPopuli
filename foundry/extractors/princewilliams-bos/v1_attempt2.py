...
def _coerce_max(max_meetings):
    """The runtime may hand us a raw int, or an args-like object carrying the
    value on an attribute. Resolve deterministically to a non-negative int."""
    if isinstance(max_meetings, bool):
        return 0
    if isinstance(max_meetings, int):
        return max(0, max_meetings)
    for attr in ("max_meetings", "max", "n", "limit", "count"):
        val = getattr(max_meetings, attr, None)
        if isinstance(val, int) and not isinstance(val, bool):
            return max(0, val)
    try:
        return max(0, int(max_meetings))
...
def extract(rt, max_meetings):
    max_meetings = _coerce_max(max_meetings)
