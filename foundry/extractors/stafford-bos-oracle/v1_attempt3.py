...
def _as_text(val):
    """Coerce a fetch result into a string, or return None."""
...
def _harvest_json(obj, index):
    ...
    while stack and guard < 200000:
        ...
        if isinstance(cur, dict):
            ...
            if date:
                if isinstance(url, str) and _abs_url(url):
                    index.setdefault(date, _abs_url(url))
                elif docid is not None and not isinstance(docid, (dict, list)):
                    index.setdefault(
                        date,
...
def _harvest_text(rt, url, params, index):
    ...
    txt = _as_text(_safe_text(rt, url, params))
    if not txt:
        return
    stripped = txt.strip()
    if stripped[:1] in ("{", "["):
...
def extract(rt, args):
    ...
    for date in dates:
        ...
        if covered >= max_meetings:
            # Document exists but we already have enough coverage.
            continue
