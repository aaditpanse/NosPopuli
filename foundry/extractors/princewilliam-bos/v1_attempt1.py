...
def _action_from_motion(motion, mapped_result):
    if motion:
        ...
        if m:
            ...
            # trim at "Moved" if the regex over-ran
...
def _enumerate_meetings(rt, max_meetings):
    """Page through the VoteLog search API, newest first."""
    collected = []
    seen = set()
    page = 1
    max_pages = 30
    want = max(1, int(max_meetings)) if max_meetings else 1
...
def extract(rt, max_meetings):
    ...
    raw_meetings = _enumerate_meetings(rt, max_meetings)

    # Parse dates; keep only meetings with a usable date and votes; newest first.
    dated = []
    ...
    want = max(1, int(max_meetings)) if max_meetings else 0
    dated = dated[:want] if want else dated
    ...
    for date, m in dated:
        try:
            ...
            if meeting_id in used_meeting_ids:
                # collision on same date (rare) -> disambiguate with clip
            ...
            if not attendance:
                # No roster derivable -> skip meeting rather than emit invalid.
                continue
            ...
            for idx, agenda_text, motion, result_raw, positions, counts in parsed_votes:
                ...
                # Emit vote_event only when there is a clear outcome and
                # per-member positions were recorded.
            ...
            # Robustness: skip any meeting that fails to parse.
            continue
