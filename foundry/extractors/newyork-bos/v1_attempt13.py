...
Votes/tallies come from the public minutes document (View.ashx PDF -> text).
Descriptive SUBJECT titles come from the MeetingDetail HTML grid
(File# / Name / Title cells), keyed by file number — the coupled-vote
sections of the minutes only print "file# + motion + tally", so titles must
come from the detail grid, not the motion text.
...
_STRIP_WORDS = frozenset((
    ...
    "failed", "preconsidered", "land", "use", "matter", "number", "council",
    "committee", "roll", "call", "motion", "made", "filed", "referred",
    "ver", "type", "action", "result", "details", "version", "agenda",
    "prime", "sponsor", "sponsors",
))
...
def _find_legis_title(seg):
    """Secondary: descriptive legislative title inside the minutes segment."""
...
def _seg_vote(seg):
    ...
    for j in range(start_idx, len(labels)):
        ...
        if len(names) != n:
            return None
        counts[p] = counts.get(p, 0) + n     # includes explicit zeros
        for nm in names:
...
def _titles_from_detail_html(html_text):
    """Map file# key -> descriptive subject title from the meeting grid."""
    titles = {}
    if not html_text:
        return titles
    for rm in re.finditer(r'<tr[^>]*>(.*?)</tr>', html_text, re.S | re.I):
        ...
        best = ''
        for c in cands:
            if not c or _looks_file(c):
                continue
            if not _has_subject(c):
                continue
            if len(c) > len(best):
                best = c
        if best:
            best = best[:240]
            if key not in titles or len(best) > len(titles[key]):
                titles[key] = best
    return titles
...
def extract(rt, args):
    ...
    for cand in past:
        ...
        try:
            ...
            # titles from the meeting detail grid
            ...
            for it in parsed:
                title = html_titles.get(it["key"])
                if not title or not _has_subject(title):
                    title = it["minutes_title"]
                if not title or not _has_subject(title):
                    continue  # no real subject -> drop rather than fabricate
                ...
                if qkey in seen_quotes:
                    continue      # coupled roll call reprinted per bill
            ...
            if not local_items or not local_votes:
                continue
