...
Roll-call format in these minutes (PDF/HTML -> text):
   Affirmative: 51 - Abreu, Ariola, ... and Zhuang
   Negative:    0
   Absent:      2 - Name, Name
The stated count is authoritative; names are comma/"and" separated.

Duplicate-quote fix: in batch meetings (e.g. budget adoption) many distinct
resolutions carry the SAME unanimous roll call, so a quote taken from the
roll-call text alone is identical across items. Each evidence quote is
therefore anchored at the item's nearest preceding file-number heading, which
uniquely identifies the motion; a global dedupe net drops any residual
collision rather than emitting two votes with the same evidence.
...
# Item heading / file-number anchor used to make each vote's evidence unique.
_FILE_ANCHOR_RE = re.compile(
    r'\b(?:Preconsidered\s+)?'
    r'(?:(?:Int|Res|Introduction|Resolution|L\.?\s?U|SLR)\.?\s*'
    r'(?:No\.?\s*)?\d{1,4}-\d{2,4}[A-Za-z0-9\-]*'
    r'|T\d{4}-\d+|M\d{2,}-\d+)',
    re.I)
...
def _nearest_file_number(text, block_start):
    ...
    if not last:
        return None
    num = re.search(r'\d{2,4}-\d{4}(-S\d+)?', last.group(0))
...
def _build_quote(text, block_start, head_end, seen):
    """Anchor the evidence quote at the nearest preceding item heading so it
    uniquely identifies this motion; fall back to the roll-call slice. Return
    None if every candidate collides with an already-emitted quote."""
    lo = max(0, block_start - 1600)
    anchors = sorted(
        {lo + m.start() for m in _FILE_ANCHOR_RE.finditer(text[lo:block_start])},
...
def extract(rt, args):
    ...
    members = {}
    used_ids = set()
    seen_quotes = set()          # global dedupe net for evidence quotes
    emitted = 0
    examined = 0
    fetches = 0
    ...
    for cand in past:
        ...
        try:
            ...
            for blk in blocks:
                ...
                bstart = blk["start"]
                hm = re.match(r'[^\r\n]{0,40}?\d+',
                              text[bstart:bstart + 60])
                head_end = bstart + (hm.end() if hm else 24)

                quote = _build_quote(text, bstart, head_end, seen_quotes)
                if quote is None:
                    # residual duplicate quote — skip rather than emit a
                    # second vote sharing another's evidence.
                    continue

                file_number = _nearest_file_number(text, bstart)
