...
("A Local Law to amend ...", "Resolution approving ...") followed by
Sponsors/Attachments and then the motion + roll-call tally:
   ... by the following vote: Affirmative: 51 - Abreu, ... and Zhuang
   Negative: 0 Abstention: 0 Absent: 2 - Name, Name

Coupled votes (one motion covering many bills) reprint the identical
tally under each bill; those are ONE roll call and are deduped by their
...
# Motion / vote block: terminates the region we search for the title.
_MOTION_CUT_RE = re.compile(
    r'(A motion\b|This\s+[A-Z][A-Za-z]+\s+was\b|By the following vote|'
    r'Affirmative\s*[:0-9]|Negative\s*[:0-9]|In Favor\b|By Council\b)', re.I)

# Ends a legislative title (whatever follows the subject heading).
_TITLE_END_RE = re.compile(
    r'(Sponsors?\s*:|Attachments?\s*:|Indexes?\s*:|Enactment\b|'
    r'A motion\b|This\s+[A-Z][A-Za-z]+\s+was\b|By the following vote|'
    r'Affirmative\b|Negative\b|In Favor\b)', re.I)

# Where an NYC legislative title begins.
_TITLE_KW_RE = re.compile(
    r'(A Local Law\b|A LOCAL LAW\b|Local Law to\b|'
    r'A Resolution\b|Resolution (?=[a-z])|Application\b)')
...
_ATTACH_INDICATORS = re.compile(
    ...
    r"Mayor's Message|Hearing Held|Laid Over|Hearing Testimony",
    re.I)
...
def _seg_title(seg, anchor, result):
    ...
    Search the region before the motion for the title keyword; take from
    there to the first Sponsors/Attachments marker. Never the motion or
    tally text, never an attachment list.
    """
    body = seg[len(anchor.group(0)):]
    mc = _MOTION_CUT_RE.search(body)
    region = body[:mc.start()] if mc else body[:800]
    region = re.sub(r'\s+', ' ', region).strip()

    def _finish(tail):
        te = _TITLE_END_RE.search(tail)
        cand = (tail[:te.start()] if te else tail)
        cand = re.sub(r'\s+', ' ', cand).strip(' .,;:*~\u2013\u2014-')
        if len(cand) >= 8 and not _ATTACH_INDICATORS.search(cand):
            return cand[:220]
        return None

    km = _TITLE_KW_RE.search(region)
    if km:
        got = _finish(region[km.start():])
        if got:
            return got

    tl = re.search(r'\bTitle\s*:\s*(.+)', region, re.I)
    if tl:
        got = _finish(tl.group(1))
        if got:
            return got

    # cleaned remainder: strip leading metadata labels, then take to marker.
    rem = re.sub(
        r'^(?:\s*(?:Version|Name|Type|Status|File\s*#|On agenda|'
        r'Final action|In control|Enactment date|Enactment|Agenda note|'
        r'Ver|Prime sponsor)\s*[:#]?\s*\S[^:]*?(?=\s+[A-Z][a-z]+\s*:|$))+',
        '', region, flags=re.I).strip(' *.,;:\u2013\u2014-')
    got = _finish(rem)
    if got:
        return got
