...
Pipeline (all confirmed against the live fetch trace):
  1. Enumerate meetings from NYC Open Data (Socrata m48u-yjt8). Fields are
     event_id, committee, meeting_date, meeting_time, meeting_location,
     agenda_status, minutes_status, and URL fields (one of which is the
     Legistar MeetingDetail page). Server-side $order/$where on event_date
     is NOT supported (returns empty) — the date field is `meeting_date`;
     we sort/filter client-side.
  2. For each past meeting, use its MeetingDetail page URL (human-viewable
     source_url); fetch that HTML only to locate the "Minutes" View.ashx
     link, then fetch the minutes document as text (runtime converts PDF).
  3. NYC Legistar minutes record roll-call votes as verbatim tally lines
     ("Affirmative: 46 - Abreu, Ariola, ...  Negative: 2 - Holden, ...").
     Parse those into per-member positions (capturing dissent/absences),
     derive attendance from the same rosters, attach the verbatim passage
     as `evidence`.
...
def _clean_piece(piece):
    ...
    if not p:
        return ("skip", None)
    # cut at any sentence/keyword marker
    cut = _STOP_TITLE.split(p)[0]
    # strip trailing '- 46' and leading '46 -'
...
def _parse_minutes(text):
    ...
    # attendance from every entry that named people
    ...
    # group into vote blocks (aye-led)
    blocks = []
    cur = None
    prev_start = None
    for e in entries:
...
def _pick_insite(urls):
    for u in urls:
        if "meetingdetail" in u.lower():
            return u
    for u in urls:
        low = u.lower()
        if "legistar" in low and ".ashx" not in low and ".asmx" not in low:
            return u
    return None


def _pick_minutes(urls):
    for u in urls:
        low = u.lower()
        if "view.ashx" in low and "m=m" in low:
            return _abs_url(u)
    return None
...
def _fetch_socrata(rt):
    variants = [
        {"$order": "meeting_date DESC", "$limit": "600"},
        {"$limit": "600"},
        None,
    ]
...
def extract(rt, args):
    ...
    for date, row in dated:
        ...
        try:
            urls = _row_urls(row)
            insite = _pick_insite(urls)
            murl = _pick_minutes(urls)
            ...
            body = _cikey(row, "committee", "event_body_name", "name",
            ...
            if meeting_id in used_ids:
                ev = _cikey(row, "event_id", "eventid", "id")
                meeting_id = "%s-%s" % (
                    meeting_id, _slug(ev if ev is not None else body))
                base = meeting_id
                n = 2
            ...
            # build vote events / agenda items
            vi = 0
