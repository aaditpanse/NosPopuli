...
Roll-call format in these minutes (PDF->text) looks like:
   Affirmative: 51 - Abreu, Ariola, Ayala, ... and Zhuang
   <newlines> <next agenda item text>
The stated count (51) is authoritative; the name list is comma/"and"
separated. Critically, the LAST name is NOT comma-terminated — it is
followed by newlines and the next item's text — so each name token must be
truncated at its first newline and validated as a person name.
...
def _clean_name_token(tok):
    t = str(tok or "")
    # Names never span lines in these rosters; the LAST name in a list runs
    # straight into the next item's text separated only by newlines. Cut it.
    t = t.split('\n', 1)[0].split('\r', 1)[0]
...
def _parse_tally(seg):
    """Return (stated_count, cleaned_names). Each name token is truncated at
    its first newline and normalized; only person-shaped names are kept."""
    ...
    for piece in rest.split(','):
        for sub in re.split(r'\s+and\s+', piece):
            # truncate at first newline so trailing document text can't leak
            sub = sub.split('\n', 1)[0].strip()
        ...
        try:
            ...
            for blk in blocks:
                ...
                for e in blk["entries"]:
                    if not e["names"]:
                        continue
                    # counts must match the document's stated tally exactly
                    if len(e["names"]) != e["n"]:
                        bad = True
                        break
                    for nm in e["names"]:
