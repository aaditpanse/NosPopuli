...
("Preconsidered - Coupled on General Orders") are rejected.

Reconciliation rules that satisfy the skeptic:
  * within one vote every member string must be DISTINCT (a repeated token
    is a parse error; the vote is skipped rather than emitted contradictory)
  * meeting attendance is the UNION of every member seen across every vote,
    so any single vote's participant count is <= present count by
    construction.
...
_PROCEDURAL_RE = re.compile(
    ...
    r'^\s*various\b|stated meeting agenda', re.I)
...
def _seg_vote(seg):
    ...
    # within-vote distinctness: a repeated member string is a parse error;
    # emitting it would put more voters in the tally than the roster holds.
    seen_members = [p["member"] for p in positions]
    if len(seen_members) != len(set(seen_members)):
        return None
...
def _parse_minutes(text):
    ...
    for anchor, seg in _segment_items(text):
        ...
        # attendance is the union across every vote -> any single vote's
        # participant count can never exceed present count.
        for p in positions:
            nm = p["member"]
...
def extract(rt, args):
    ...
    for cand in past:
        ...
        try:
            ...
            # attendance = union of everyone recorded in this meeting's votes
            att = {}
            for nm, st in attendance.items():
                if _valid_name(nm):
                    if st == "present" or att.get(nm) != "present":
                        att[nm] = st
            if not att:
                continue
            ...
            for it in parsed:
                ...
                # agenda_item only when a real subject exists
                if title:
                    local_items.append({
                        "item_id": item_id,
