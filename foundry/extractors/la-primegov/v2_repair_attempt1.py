...
def extract(rt, meeting_ids):
    ...
    for raw_mid in meeting_ids:
        ...
        for i, m in enumerate(matches):
            ...
            # All-zero blocks mean no vote was taken; drop them so they never
            # become vote_events and never influence the item's result.
            votes = [
                vm for vm in _VOTE_RE.finditer(body) if not _is_all_zero(vm)
            ]
            item_votes = []
